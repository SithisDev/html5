(function() {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://z01.online/js/nws-client-es5.js"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('http://z01.online/player-inner/' + element.url);
      return;
    }
        ...
        items.forEach(function(element, index) {
          ...
          if (element.quality) {
			for (var q in element.quality) {
				if (parseInt(q) > 720) {
					delete element.quality[q];
				}
			}

			var keys = Lampa.Arrays.getKeys(element.quality);

			if (keys.length > 0) {
				element.qualitys = element.quality;
				element.quality = keys[0];
			} else {
				element.quality = null;
			}
		}
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '',
      name: 'Z01',
      description: 'Бесплатный онлайн кинотеатр',
      component: 'lampac_z01',
